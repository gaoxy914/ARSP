...
bool HyperRect::upper_dominate(const Point& point) const {
    bool unique = false;
    for (int i = 0; i < m_dim; ++ i) {
        if (m_upper[i] > point[i]) {
            return false;
        } else if (m_upper[i] < point[i]) {
            unique = true;
        }
    }
    return unique;
...
bool HyperRect::lower_dominate(const Point& point) const {
    bool unique = false;
    for (int i = 0; i < m_dim; ++ i) {
        if (m_lower[i] > point[i]) {
            return false;
        } else if (m_lower[i] < point[i]) {
            unique = true;
        }
    }
    return unique;
...
HyperRect* HyperRect::get_subrect(const int& k) {
    HyperRect *sub_rect = new HyperRect(m_dim);
    for (int i = 0; i < m_dim; ++ i) {
