...
bool Instance::dominate_larger(const HyperRect& rect) const {
    bool unique = false;
    for (int i = 0; i < m_point.m_dim; ++ i) {
...
bool Instance::cross_larger(const HyperRect& rect) const {
    bool unique = false;
    for (int i = 0; i < m_point.m_dim; ++ i) {
        if (m_point[i] < rect.m_lower[i]) {
            return false;
...
bool Instance::fdominate_V_larger(const Instance& other, const vector<Point>& vertex) const {
    bool unique = false;
    double score_t = 0, score_s = 0;
    for (int i = 0; i < vertex.size(); ++ i) {
        score_t = score(vertex[i]);
