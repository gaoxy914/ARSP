...
class DataOperator {
    ...
    void LoadData(const char *path) {
        ifstream file((string(path) + string("/cnt.data")).c_str(), ios::in);
        file >> m;
        ins_cnt = new int[m];
        for (int i = 0; i < m; ++ i) { file >> ins_cnt[i]; n += ins_cnt[i]; }
        file.close();
        file.open((string(path) + string("/instances.data")).c_str(), ios::in);
        for (int i = 0; i < m; ++ i) {
    ...
    void WriteData(const char *path) {
        ofstream file((string(path) + string("/cnt.data")).c_str(), ios::out);
        file << m;
        for (int i = 0; i < m; ++ i) file << ins_cnt[i];
        file.close();
        file.open((string(path) + string("/instances.data")).c_str(), ios::out);
        for (int i = 0; i < m; ++ i) {
            for (int j = 0; j < ins_cnt[i]; ++ j) {
                file << dataset[i][j].obj_id;
                file << dataset[i][j].ins_id;
